...
var (
	imageDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Filename`: `text`, `Href`: `text`, `CreatorID`: `integer`}
	_            = bytes.MinRead
)
